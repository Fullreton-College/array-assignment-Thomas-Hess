...
int main()
{
     ...
     do {
          ...
          cin >> c;
          cin.ignore(); // not sure what this does, but I'm not touching it
     } while(c == 'y');
...
string lookupName(string targetName, string names[], string phoneNumbers[], int size) {
    for (int i = 0; i < size; i++) {
        if (names[i] == targetName) {
            return phoneNumbers[i]; // when input is found in all names,
                                    // then return corresponding phone number
        }
    }
